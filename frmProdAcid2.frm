...
Begin VB.Form frmProdAcid2 
   ...
   Begin VB.PictureBox Picture2 
      ...
      Begin MSComCtl2.DTPicker tgl 
         ...
         CurrentDate     =   39419
      End
      Begin VB.CommandButton cmdRekomendasi 
         Enabled         =   0   'False
         BeginProperty Font 
            Name            =   "Trebuchet MS"
            Size            =   8.25
      ...
      Begin VB.OptionButton OptKotor 
         ...
         Width           =   855
      End
      Begin TrueOleDBGrid80.TDBGrid tgAcid 
         Height          =   3495
         Left            =   120
         TabIndex        =   12
         Top             =   2400
   ...
   End
End
Attribute VB_Name = "frmProdAcid2"
...
Private Function OpenPartner(ByVal Index As Integer) As Boolean
    ...
    Else
        ...
        MessageBox "Konfigurasi ACID TREATMENT masih kosong", "Peringatan", msgOkOnly, msgInfo
        OpenPartner = True
    End If
...
Private Sub cmdRekomendasi_Click()
    OpenPartner 1
End Sub
...
Private Sub mCall_RowColChange(ByVal TagForm As String, _
    ...
    Select Case TagForm
        ...
        Case "ACID"
            lblMO.Caption = mCall.GetFieldByName("RlNo")
...
Private Sub BindDataToGrid(ByVal ParameterString As String)
    ...
    RcDetail.DBOpen _
       ...
       " Where  LabRekomEkstraksi.SplNo = '" & ParameterString & "'  and LabRekomEkstraksi_Line.FORMNAME = 'ACID TREATMENT' and LabProses.kolom = '0' Order By  LabSetupRekom_Line.ProsesID ", CNN, lckLockBatch
    ...
    Set tgAcid.DataSource = MYDDE.ChildRecordset
    RcDetail.CloseDB
...
Private Sub clearColumn()
    ...
    With tgAcid
        ...
        For nrows = 0 To MYDDE.ChildRecordset.Recordcount - 1
            .Columns(2).Text = ""
            .MoveNext
        Next
...
Private Sub Form_Load()
    ...
    With MYDDE
        ...
        .PrepareQuery = "SELECT * From LabProsesProduksi_Header where type_proses='ACID'"
        .SetPermissions = aksess.MayDo("Acid Treatment")
    End With
...
Private Sub MyDDE_AfterPrepareActiveDB(ByVal AdReasonActiveDb As SemeruDC.TypeButtonData)
    ...
    Select Case AdReasonActiveDb
        ...
        Case tmbAddNew
            ...
            lblDokNo.Caption = IDGen.GetID("ACID")
            lblNoRL.Caption = frmProduksi.lblNoRL
            lblMetode.Caption = frmProduksi.lblMetode
            BindDataToGrid lblRekomendasi
            
            tgAcid.AllowUpdate = True
            tgAcid.Columns(0).Locked = True
            tgAcid.Columns(1).Locked = True
            tgAcid.Columns(2).Locked = False
        Case tmbCancel

            tgAcid.Columns(2).Locked = True
...
Private Sub SaveToMO()
    ...
    rsCek.DBOpen "select WCID from WCenter_Header where FormID = 38", CNN
...
Private Sub PrepareQuery()
    ...
    With MYDDE
        ...
        .PrepareAppend = "INSERT INTO LabProsesProduksi_Header(DokNo, NoEkstraksi, RLNo, Berat, Tanggal,[Group], Tanki, Keterangan, Kondisi,refid,type_proses, no_rekom, methode,waktu_mulai,waktu_selesai) VALUES ('" & lblDokNo.Caption & "','" & lblEkstraksi.Caption & "','" & lblNoStock.Caption & "','" & txtBerat.Text & "',convert(datetime,'" & DcTanggal.Value & "',3),'" & txtGroup.Text & "','" & txtTanki.Text & "','" & TxtKeterangan.Text & "','" & ket & "','" & lblMO.Caption & "','" & "ACID" & "','" & lblRekomendasi.Caption & "','" & lblMetode.Caption & "',convert(datetime,'" & tgl(0).Value & "',3),convert(datetime,'" & tgl(1).Value & "',3))"
        .PrepareUpdate = "UPDATE LabProsesProduksi_Header SET  DokNo = '" & lblDokNo.Caption & "',NoEkstraksi ='" & lblEkstraksi.Caption & "',refid='" & lblMO.Caption & "',RLNo ='" & lblNoStock.Caption & "',Berat ='" & txtBerat.Text & "',Tanggal = convert(datetime,'" & DcTanggal.Value & "',3),[Group] ='" & txtGroup.Text & "', Tanki ='" & txtTanki.Text & "', Keterangan ='" & TxtKeterangan.Text & "', Kondisi = '" & TxtKeterangan.Text & "',methode='" & lblMetode.Caption & "' where labprosesproduksi_header.dokno='" & .GetFieldByName("dokno") & "'"
        .PrepareDelete = "DELETE From LabProsesProduksi_Header Where LabProsesProduksi_Header.DokNo = '" & .GetFieldByName("dokno") & "'"
    End With
    SendDataToServer "UPDATE StatusProduksi SET Rekomendasi = '" & lblRekomendasi.Caption & "',Posisi = '" & "ACID TREATMENT" & "', status = '1',tanggal = convert(datetime,'" & Format(Now, "dd/mm/yyyy") & "',3) Where  StatusProduksi.NoEkstraksi = '" & lblEkstraksi.Caption & "'"
    Exit Sub
Masjid:
    MessageBox Err.Description, "Peringatan", msgOkOnly, msgExclamation
    Err.Clear
...
Private Sub SimpanDetail()
    ...
    For I = 0 To MYDDE.ChildRecordset.Recordcount - 1
        SendDataToServer "INSERT INTO labprosesproduksi_line(DokNo,ProsesID,ID,kebutuhan) VALUES('" & lblDokNo.Caption & "','" & tgAcid.Columns(3).Value & "','" & tgAcid.Columns(4).Value & "','" & IIf(tgAcid.Columns("kebutuhan").Value = "", ".", tgAcid.Columns("kebutuhan").Value) & "')"
        MYDDE.ChildRecordset.MoveNext
    Next
    
    SendDataToServer "UPDATE StatusProduksi SET Rekomendasi = '" & lblDokNo.Caption & "',Posisi = '" & "ACID TREATMENT" & "', status = '1',tanggal =convert(datetime,'" & DcTanggal.Value & "',3) Where  StatusProduksi.NoEkstraksi = '" & lblEkstraksi.Caption & "'"
    Exit Sub
Masjid:
    MessageBox Err.Description, "Peringatan", msgOkOnly, msgExclamation
    Err.Clear
