...
Begin VB.Form frmVoucher 
   ...
   Begin VB.PictureBox Picture2 
      ...
      Begin VB.ComboBox cboVoucher 
         ...
         EndProperty
         Height          =   315
         ItemData        =   "frmVoucher.frx":6852
         Left            =   2692
         List            =   "frmVoucher.frx":685C
...
With MyDDE
    ...
    Set .BindForm = frmVoucher
    .BindFormTAG = "Voucher"
    .SetPermissions = UserEditDeleteDenied
    Set .ActiveConnection = CNN
    '.PrepareQuery = " SELECT     TransData.TransID, TransData.RefNotes, TransData.DateTrans, TransData.Kode Kas, PartnerDB.PartnerID,                        PartnerDB.CompanyName + ' (' + PartnerDB.ContactName + ')' + CHAR(10)                        + PartnerDB.Address + ' , ' + PartnerDB.City + ' (+Kotak Pos : ' + PartnerDB.PostalCode + ')' + + CHAR(10) AS Alamat,                        'Telp : ' + PartnerDB.Phone + CHAR(10) AS Telepon, 'Mobile : ' + PartnerDB.Mobile + CHAR(10) AS Mobile, 'Fax : ' + PartnerDB.Fax + CHAR(10)                        AS Faximile, TransData.TypeTrans, [Temp Bank].NamaBank AS [Bank Name]" & _
                    " FROM         TransData INNER JOIN                       PartnerDB ON TransData.PartnerId = PartnerDB.PartnerID INNER JOIN                       [Temp Bank] ON TransData.Kode Kas = [Temp Bank].Kode Kas WHERE     (TransData.TypeTrans = N'BP') OR                       (TransData.TypeTrans = N'BR') ORDER BY TransData.TransID"
    .PrepareQuery = " SELECT TransData.TransID, TransData.RefNotes, TransData.DateTrans, " & _
                    ...
                    " GLAccount ON TransData.bankid = GLAccount.NoAccount WHERE (TransData.TypeTrans = N'BP') OR " & _
                    " (TransData.TypeTrans = N'BR') OR (TransData.TypeTrans = N'BU') OR " & _
                    " (TransData.TypeTrans = N'BK') ORDER BY TransData.TransID"
                    
End With
Set mCall = New frmCaller
If UCase(AINode.Key) = "VOUCHERTRANSAKSI" Then
    cboVoucher.ListIndex = 0
    Me.Caption = "Voucher Pembelian"
Else
    cboVoucher.ListIndex = 1
    Me.Caption = "Voucher Penjualan"
End If
End Sub
...
Select Case Index
       ...
       Case 1:
            ...
            ElseIf cboVoucher = "Pelunasan Piutang" Then
               RcPartner.DBOpen "SELECT     [Voucher Batch].PartnerID AS [Kode Customer], PartnerDB.CompanyName AS Nama, PartnerDB.Address AS Alamat, PartnerDB.City AS Kota,                       PartnerDB.Phone AS Telepon FROM         [Voucher Batch] INNER JOIN                       PartnerDB ON [Voucher Batch].PartnerID = PartnerDB.PartnerID GROUP BY [Voucher Batch].PartnerID, PartnerDB.CompanyName, PartnerDB.Address, PartnerDB.City, PartnerDB.Phone, [Voucher Batch].TypeTrans HAVING      ([Voucher Batch].TypeTrans = N'AR') ORDER BY [Voucher Batch].PartnerID", CNN, lckLockReadOnly
            End If
...
Select Case cboVoucher
       ...
       Case "Pelunasan Piutang":
             If SendDataToServer(" INSERT INTO [Table Journal]" & _
                ...
                SendDataToServer (" INSERT INTO [Detail Journal]" & _
                                  " (JournalID, NoAccount, [Doc Reff], Debet, Credit) " & _
                                  " VALUES   (N'" & mykey & "', N'" & CariTypeAccount(39) & "', N'" & NoVoucher(2) & "', " & CCur(lblABayar) & ", 0)")
