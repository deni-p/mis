...
strSource = GetSetting("Manufacturing Intelligent", "Data", "Patch Location")
FileVer = ""
DisplayVerInfo (strSource)

ServerFile = FileVer

strTarget = App.Path & "\" & "Manufacturing Intelligent.exe"
FileVer = ""
DisplayVerInfo (strTarget)
LocalFile = FileVer

If ServerFile > LocalFile Then
   Shell App.Path + "\patch.EXE", vbNormalFocus
   'End
End If
