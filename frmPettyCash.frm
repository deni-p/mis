...
Begin VB.Form FrmPettyCash 
   ...
   Caption         =   "Pembelian Langsung (Petty Cash)"
   ClientHeight    =   6945
   ClientLeft      =   1635
   ClientTop       =   1920
   ClientWidth     =   11430
...
If MEdit = True Then If MyDDE.ChildRecordset.Recordcount <> 0 Then MyDDE.ChildRecordset.Fields("ScheduleDate").Value = DTPicker1.Value
End Sub
...
SQLInit = " SELECT [PO Order].PurchaseID, [PO Order].PartnerID, [PO Order].Kurs, [PO Order].DatePurchase, [PO Order].TermPayment, [PO Order].Taxes, [PO Order].Status, [PO Order].Periode, [PO Order].TypeTrans, [PO Order].TypeLoco, PartnerDB.CompanyName, PartnerDB.Address, PartnerDB.City, [PO Order].Account, [PO Order].Discount,[PO Order].StatusSJ,[PO Order].[Require Date], [PO Order].TermMethod, [PO Order].keterangan,[PO Order].person  FROM  [PO Order] INNER JOIN   PartnerDB ON [PO Order].PartnerID = PartnerDB.PartnerID WHERE "
'GridLayout
...
'      .PrepareQuery = SQLInit & "([PO Order].StatusSJ = 0) AND (LEFT([PO Order].PurchaseID, 2) = 'PO') ORDER BY [PO Order].PurchaseID"
      .PrepareQuery = SQLInit & "TypeTRans='PO' and type_trans_order=4 ORDER BY [PO Order].PurchaseID"
     Else
      .PrepareQuery = SQLInit & "[PO Order].PurchaseID ='" & pWhere & "' and TypeTRans ='PO' and type_trans_order=4 "
     End If
...
Select Case TagForm:
       ...
       Case "Inventory List", "Remindier":
            ...
            MyDDE.ChildRecordset.Fields("tipe_item") = "I"
            MyDDE.ChildRecordset.Fields("TMP") = mCall.GetFieldByName("PriceIn")
            MyDDE.ChildRecordset.Fields("lokasiGdg") = mCall.GetFieldByName("lokasiGdg")
            If Not IsEmpty(mCall.GetFieldByName("CurrID")) Then
               MyDDE.ChildRecordset.Fields("CurID") = mCall.GetFieldByName("CurrID")
               MyDDE.ChildRecordset.Fields("Rate") = mCall.GetFieldByName("Rate")
            Else
         ...
         Case "No Perkiraan"
            MyDDE.GetFieldByName("account") = mCall.GetFieldByName("Kode Kelompok Gudang")
            'txtBox(2).Text = mCall.GetFieldByName("NoAccount")
...
Select Case AdReasonActiveDb
       Case tmbAddNew:
            ...
            rsSupplier.DBOpen "select partnerid from partnerDB where [default] = 1", CNN
            If rsSupplier.DBRecordset.Recordcount > 0 Then
               MyDDE.GetFieldByName("partnerID") = rsSupplier.DBRecordset.Fields("PartnerID")
            Else
               MyDDE.GetFieldByName("partnerID") = ""
            End If
            ...
            newPO = IDGen.GetID("PC")   'MyData.PrepareIndex(tmbTransaksiPO, 5, "1", TglIndex)
       ...
       Case tmbPrint:
            If doPrint Then
                  ...
                  aReport.CallReportView "select * from reportPurchasing where PurchaseID='" & MyDDE.GetFieldByName("PurchaseId") & "'", "ReportPurchasing.rpt", ReportPath, "Purchase Order"
                  Set aReport = Nothing
               doPrint = False
            End If
            SetLabelStatus
...
Private Sub SimulasiPenerimaan()
    Dim oIDTRans As New IDGenerator
    IDTRans = oIDTRans.GetID("PC")
    SendDataToServer "insert into TransData (TransID     ,EmpID                                       ,DateTRans                         ,status,PurchaseID         ,TypeTRans,statusInvoice,CurrID,PartnerID,warehouse) values ('" & _
                                             IDTRans & "','" & MainMenu.StatusBar1.Panels(1).Text & "','" & Format(Now, "yyyy-MM-dd") & "',0     ,'" & txtBox(0) & "','AP'     ,0            ,'IDR','" & MyDDE.GetFieldByName("PartnerID") & "',null)"
                                    
    Set oIDTRans = Nothing
End Sub
...
Select Case Index
       ...
       Case 3:  'detail order normal
            RcPartner.DBOpen "SELECT NoItem AS [No Barang], ItemName AS [Nama Barang], UOMPurchase as uom, PPn,PriceIn AS Harga,inventory.CurrID,[Currency Setup].rate,warehouse as lokasiGdg FROM Inventory left outer join [currency Setup] on Inventory.currid = [currency setup].currID WHERE (Manufacture = 0) ORDER BY NoItem", CNN, lckLockReadOnly
            mFirstCaller = True
       Case 4:  'termpayment
       ...
       Case 7:
            RcPartner.DBOpen "SELECT GLAccount.NoAccount AS [Kode Kelompok Gudang], GLAccount.AccountName AS [Nama Kelompok Gudang] FROM         GLAccount INNER JOIN                       AccType ON GLAccount.Type = AccType.Tipe WHERE     (AccType.ID = 51)", CNN
End Select
If RcPartner.Recordcount <> 0 Then
   Select Case Index
          ...
          Case 7:
            mCall.FromTagActive = "No Perkiraan"
            mCall.CaptionLink = "No Perkiraan"
...
RcDetail.DBOpen "SELECT [Detail PO].NoItem, Inventory.ItemName, Inventory.uomPurchase as uom, [Detail PO].QTYPO, [Detail PO].POPrice, [Detail PO].VAT, [Detail PO].ScheduleDate,  " & _
       ...
       "[Detail PO].PurchaseID , [Detail PO].QTYTemp, [Detail PO].StatusTrans, [Detail PO].SPPID, [Detail PO].tipe_item, [Detail PO].curID, [Detail PO].Rate,inventory.warehouse as lokasiGdg " & _
       " FROM [Detail PO] INNER JOIN  Inventory ON [Detail PO].NoItem = Inventory.NoItem INNER JOIN [PO Order] ON [Detail PO].PurchaseID = [PO Order].PurchaseID WHERE     ([Detail PO].PurchaseID = N'" & ParameterString & "') ORDER BY [Detail PO].NoItem", CNN, lckLockBatch
     ...
     If .Recordcount <> 0 Then
           ...
           Do
              If .EOF = True Then Exit Do
                '*** simpan ke table Detail PO ***'
                SendDataToServer " INSERT INTO [Detail PO] ( PurchaseID, NoItem, QTYPO, ItemSupplierID, POPrice, ScheduleDate, VAT,QtyTemp,TCredit,Hpp,sppid,tipe_item,curID,rate,statusTrans,DNID)" & _
                                 " VALUES (N'" & txtBox(0) & "', N'" & .Fields("NoItem") & "', " & FQty(.Fields("QTYPO")) & ", N'" & MyDDE.GetFieldByName("PartnerID") & "', " & FQty(.Fields("POPrice")) & ", convert(Datetime,'" & Format(.Fields("ScheduleDate"), "dd/mm/yy") & "',3), " & CDbl(.Fields("VAT")) & ", " & FQty(.Fields("QTYPO")) & "," & CCur(LblAmount(3)) & "," & FQty(.Fields("POPrice")) & ",'" & .Fields("sppid") & "','" & .Fields("tipe_item") & "','" & .Fields("curID") & "'," & FQty(.Fields("rate")) & ",2,'" & IDTRans & "')"
                
                '*** Simpan ke table Inventor Stock / Update stock gudang ***'
                SendDataToServer "insert into [inventory tabel] (noIdx ,NoItem                     ,Qty_in                        ,PriceIn                         ,Qty_out,RefTRans           ,DateTrans                         ,StockTmp                  ,LockFIFO,TypeTrans,HPP) values (" & _
                                                               "newID(),'" & .Fields("NoItem") & "'," & FQty(.Fields("QTYPO")) & "," & FQty(.Fields("POPrice")) & ",0      ,'" & txtBox(0) & "','" & Format(Now, "yyyy-MM-dd") & "'," & FQty(.Fields("QTYPO")) & ",0   ,'AP'     ," & FQty(.Fields("POPrice")) & ")"
                                                               
              
                '*** Simpan ke table TransData untuk simulasi penerimaan barang ***'
'                SendDataToServer "Insert into [Detail TRansData] (Idx,TransID,  noItem                    ,DateTrans                         ,Qty_IN,Qty_receive,status) values (newID(),'" & _
'                                                                 IDTRans & "','" & .Fields("noItem") & "','" & Format(Now, "yyyy-MM-dd") & "'," & FQty(.Fields("QTYPO")) & "," & FQty(.Fields("QTYPO")) & ",0)"
               
               
                SendDataToServer " INSERT INTO [Detail TransData](Idx    ,TransID           , NoItem                      ,QTY_IN                         , QTY_Receive                   , Price                           ,Vat,StatusItem,dnid                ,Referense)" & _
                                                        " VALUES (newID(),N'" & IDTRans & "', N'" & .Fields("NoItem") & "', " & FQty(.Fields("QTYPO")) & ", " & FQty(.Fields("QTYPO")) & ", " & FQty(.Fields("POPrice")) & ",0, N'Fixed'   ,N'" & txtBox(0) & "',N'" & txtBox(0) & "')"

               
                '*** Update status po Order = 1 (close) ***
                SendDataToServer "update [po Order] set status=1 where purchaseID='" & txtBox(0) & "'"
                  
                SendAPItem .Fields("NoItem"), FQty(.Fields("QTYPO")), FQty(.Fields("POPrice")), IDTRans, Now, "RN", 1, 0, , .Fields("lokasiGdg")
...
With MyDDE
      mAccount = txtBox(5)
      strSQL = " INSERT INTO  [PO Order] ( [REquire Date]            ,PurchaseID               ,EmpID                                        , PartnerID                                   ,DatePurchase                                   , TermPayment ,  Periode                           , TypeTrans              ,Account      ,Discount               ,termMethod,keterangan          , status, Person,type_trans_order ) " & _
                        " VALUES ('" & Format(Now, "yyyy-MM-dd") & "',N'" & txtBox(0).Text & "',N'" & MainMenu.StatusBar1.Panels(1).Text & "', N'" & MyDDE.GetFieldByName("PartnerID") & "','" & Format(DTPicker1.Value, "yyyy-MM-dd") & "', 0           , " & Val(Month(DTPicker1.Value)) & ", N'" & mPoSc & "',N'" & mAccount & "' ," & FQty(txtBox(2)) & ",'CASH'    ,'" & Text1.Text & "',1      ,'" & MyDDE.GetFieldByName("Person") & "',4)"
   
    .PrepareAppend = strSQL
                     
      strSQL = " UPDATE [PO Order]" & _
                       " Set [Require Date] = Convert(datetime,'" & Format(Now, "dd/mm/yy") & "',3), empID=N'" & MainMenu.StatusBar1.Panels(1).Text & "', PartnerID = N'" & MyDDE.GetFieldByName("PartnerID") & "', DatePurchase = convert(Datetime, '" & Format(DTPicker1.Value, "dd/mm/yy") & "',3), TermPayment = 0, Periode = " & Val(Month(DTPicker1.Value)) & ", TypeTrans = N'" & mPoSc & "',Account=N'" & mAccount & "'" & _
                       ", Discount=" & FQty(txtBox(2)) & ", keterangan ='" & Text1.Text & "',person='" & MyDDE.GetFieldByName("Person") & "' WHERE     (PurchaseID = N'" & txtBox(0) & "') AND (Status = 0)"
'    Debug.Print strSQL
